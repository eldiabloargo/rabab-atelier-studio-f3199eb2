    ...
    <>
      <nav className="fixed top-0 left-0 right-0 bg-white/80 backdrop-blur-md border-b border-stone-100 z-[90] h-20">
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-full flex items-center justify-between">
          ...
          <button className="md:hidden p-2 text-stone-600" onClick={() => setMenuOpen(!isMenuOpen)}>
            {isMenuOpen ? <X /> : <Menu />}
          </button>
          ...
          <div className="hidden md:flex items-center gap-8">
            <NavLink to="/" className="text-sm font-medium text-stone-500" activeClassName="text-amber-600">
              {isArabic ? "الرئيسية" : "Accueil"}
            </NavLink>
            <NavLink to="/collection" className="text-sm font-medium text-stone-500" activeClassName="text-amber-600">
