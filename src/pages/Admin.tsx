            ...
            {!showForm ? (
               <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                 ...
                 {products?.map(p => (
                   <div key={p.id} className="relative aspect-[4/5] bg-white rounded-3xl overflow-hidden border border-stone-100 group shadow-sm">
                     ...
                     <div className="absolute inset-0 bg-stone-900/60 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-2 transition-all">
                       <Button size="icon" className="rounded-full w-8 h-8 bg-white text-stone-900" onClick={() => { setForm({...p, images_gallery: p.images_gallery || []}); setEditingId(p.id); setShowForm(true); }}><Pencil className="w-3 h-3" /></Button>
                ...
                <motion.form initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onSubmit={async (e) => {
                    ...
                    // تنظيف الصور الفارغة من الجاليري قبل الحفظ
                    const cleanGallery = form.images_gallery.filter(url => url.trim() !== "");
                    payload.images_gallery = cleanGallery;
                    ...
                    <div className="grid md:grid-cols-2 gap-6 pt-4 border-t border-stone-50">
                        <div className="space-y-3">
                            ...
                            {/* نظام الغاليري المطور */}
                            <div className="space-y-2">
                                {form.images_gallery?.map((url, index) => (
                                    <div key={index} className="flex gap-2">
                                        <Input 
                                            placeholder={`URL Galerie ${index + 1}`} 
                                            value={url} 
                                            onChange={e => {
                                            ...
                                            className="h-10 rounded-lg bg-stone-50 border-none px-4 text-[10px] flex-1" 
                                        />
                                        <Button 
                                            type="button" 
                                            onClick={() => {
                                            ...
                                            className="h-10 w-10 bg-red-50 text-red-500 rounded-lg"
                                        >
                                            <X className="w-3 h-3" />
                                        </Button>
                                    </div>
                                ...
                                <Button 
                                    type="button" 
                                    onClick={() => setForm({...form, images_gallery: [...(form.images_gallery || []), ""]})} 
                                    className="w-full h-10 border-dashed border border-stone-200 bg-transparent text-stone-400 hover:text-stone-600 rounded-lg flex items-center justify-center gap-2 transition-all"
                                >
                                    <Plus className="w-3 h-3" />
                                    <span className="text-[8px] font-black uppercase tracking-widest">Ajouter une image</span>
                                </Button>
                            </div>
                        </div>
                        <div className="space-y-3">
