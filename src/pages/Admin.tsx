...
export const Admin = () => {
  ...
  return (
    <div className="py-6 px-4 max-w-5xl mx-auto font-sans bg-[#fafaf9] min-h-screen">
      {/* Header Controls */}
      <div className="flex items-center justify-between mb-8 gap-4">
        <Link to="/" className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white border border-stone-100 text-stone-600 hover:text-stone-900 transition-colors shadow-sm">
          <ExternalLink className="w-3.5 h-3.5" />
          <span className="text-[8px] font-black uppercase tracking-widest">View Site</span>
        </Link>

        <div className="flex bg-white/50 p-1 rounded-xl border border-stone-100 shadow-sm">
          <button onClick={() => setActiveTab("products")} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${activeTab === 'products' ? 'bg-stone-900 text-white shadow-md' : 'text-stone-400 hover:text-stone-600'}`}>
            <LayoutGrid className="w-3 h-3" /> Inventaire
          </button>
          <button onClick={() => setActiveTab("categories")} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${activeTab === 'categories' ? 'bg-stone-900 text-white shadow-md' : 'text-stone-400 hover:text-stone-600'}`}>
            <Layers className="w-3 h-3" /> Collections
          </button>
        </div>

        <button onClick={() => supabase.auth.signOut()} className="p-2.5 rounded-lg bg-white border border-stone-100 text-stone-300 hover:text-red-500 transition-all shadow-sm">
